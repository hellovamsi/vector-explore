...
def run_wizard(project_root: Path) -> None:
    ...
    if embedder is None:
        ...
        if prompt_more(input, prompt="Reuse the same embedding backend/model for indexing? [Enter=yes, N+Enter=no]: "):
            pass
        else:
            embedder = _select_embedder(project_root, env, purpose="chunk embeddings (index + query)")
    ...
    run_query(store_dir=store_dir, question=question, embedder=embedder, env=env, params=qparams, input_fn=input, print_fn=print)
